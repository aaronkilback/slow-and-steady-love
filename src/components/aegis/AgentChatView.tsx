...
function getWelcomeContent(agent: AgentConfig): string {
  if (agent.id === "aegis") {
    return `**Aegis Online** — Silent Shield Security Intelligence Platform
...
How can I assist you today, Operator?`;
  ...
  return `**${agent.name} Online** — ${agent.role}
...
export function AgentChatView({ agentId = "aegis", onBack }: AgentChatViewProps) {
  const {
    conversations,
  ...
  // Fetch platform data for full agent awareness
  ...
  const Icon = agentIcons[agentId] || Shield;
...
You have full access to platform intelligence. Reference signals, team status, available agents, and locations when relevant. Continue conversations naturally.`;
  }, [messages, currentConversationId, signals, locations, profiles, agents, agentConfig]);
  ...
  useEffect(() => {
    if (currentConversationId) setTimeout(scrollToBottom, 400);
    ...
    <div className="flex h-[calc(100vh-5rem)] flex-col">
      ...
      <div className="flex items-center justify-between border-b border-border bg-card/50 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-foreground">{agentConfig.name}</h2>
            <p className="text-xs text-muted-foreground">{agentConfig.role}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Sheet>
            ...
            <SheetContent>
              <SheetHeader>
                <SheetTitle>{agentConfig.name} History</SheetTitle>
              </SheetHeader>
                ...
                <div className="space-y-2 pr-4">
                  ...
                  {conversations.length === 0 && (
                    <p className="text-center text-muted-foreground py-8">
                      No previous conversations with {agentConfig.name}
                    </p>
                  )}
                </div>
              </ScrollArea>
          ...
          {isStreaming && displayMessages[displayMessages.length - 1]?.role !== "assistant" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
              <div className="flex items-center gap-2 rounded-2xl rounded-bl-md border border-border bg-card px-4 py-3">
                ...
                <span className="text-sm text-muted-foreground">{agentConfig.name} is processing...</span>
              </div>
            </motion.div>
          )}
        </div>
      ...
      <div className="border-t border-border bg-card/50 p-4 safe-area-bottom">
        <div className="flex items-center gap-2">
          ...
          />
          <Button size="icon" onClick={handleSend} disabled={!input.trim() || isStreaming} className="shrink-0">
